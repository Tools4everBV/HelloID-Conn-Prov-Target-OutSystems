...
try {
    ...
    # Overwrite role with selected role permission
    $CurrentUser.RoleKey = $pRef.id

    Write-Verbose "Granting permission $($pRef.Name) ($($pRef.id)) to $($aRef.username) ($($aRef.id))"
    ...
    $auditLogs.Add([PSCustomObject]@{
            ...
            Message = "Successfully granted permission $($pRef.Name) ($($pRef.id)) to $($aRef.username) ($($aRef.id))"
            IsError = $false
        })
